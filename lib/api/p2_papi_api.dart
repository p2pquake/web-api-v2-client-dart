...
class P2PAPIApi {
  P2PAPIApi([ApiClient apiClient]) : apiClient = apiClient ?? defaultApiClient;
  ...
  /// * [int] offset:
  ///   読み飛ばす件数 (0以上、デフォルトは0)。例えば、 limit=100&offset=200 とすると、201件目から100件を返却します。
  Future<List<AnyOf<JMAQuake,JMATsunami,Areapeers,EEWDetection,Userquake,UserquakeEvaluation>>> historyGet({ List<int> codes, int limit, int offset }) async {
    ...
    if (response.body != null && response.statusCode != HttpStatus.noContent) {
      return (apiClient.deserialize(_decodeBodyBytes(response), 'List<AnyOf<JMAQuake,JMATsunami,Areapeers,EEWDetection,Userquake,UserquakeEvaluation>>') as List)
        .map((item) => item as AnyOf<JMAQuake,JMATsunami,Areapeers,EEWDetection,Userquake,UserquakeEvaluation>)
